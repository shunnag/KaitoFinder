...
enum ArchivePasswordLayout {
    static func stack(_ views: [NSView], width: CGFloat? = nil) -> NSStackView {
...
final class ArchivePasswordFields: NSObject, NSTextFieldDelegate {
    ...
    init(format: GyoshukuKit.ArchiveFormat, changing: Bool = false, bundle: Bundle = .main) {
        self.format = format
        self.bundle = bundle
        ...
        let labelsWidth = max(passwordLabel.intrinsicContentSize.width, verifyLabel.intrinsicContentSize.width)
        width = max(360, labelsWidth + 12 + 220 + 8,
                    methodLabel.intrinsicContentSize.width + 12 + methodPopup.intrinsicContentSize.width + 8,
        ...
        formatControl.addSubview(headersCheckbox)
        for control in [methodRow as NSView, headersCheckbox] {
            ...
            control.trailingAnchor.constraint(lessThanOrEqualTo: formatControl.trailingAnchor).isActive = true
        }
        methodRow.trailingAnchor.constraint(equalTo: formatControl.trailingAnchor).isActive = true
        formatControl.heightAnchor.constraint(equalToConstant: max(30, methodRow.fittingSize.height,
                                                                   headersCheckbox.intrinsicContentSize.height)).isActive = true
