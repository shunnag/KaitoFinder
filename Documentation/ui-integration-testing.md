...
通常の候補は`.tar.gz`などの複合拡張子を使う。拡張子まで手入力した名前の形式変更では、
名前の本体を保ち、`.tgz` / `.tbz2` / `.txz`を使って標準パネルの末尾拡張子の置き換えに合わせる。
切り替え後に名前を再編集するケースと、同じパネルで繰り返し形式を変えるケースも検査する。
操作対象は今回生成した専用 bundle ID と一致する PID に限定し、アクセシビリティの既存の許可だけを使う。
許可を要求するダイアログは出さず、利用できなければ成功扱いにしない。
通常の全件テストでは確定操作の4件を skip し、このドライバーで検証する。
