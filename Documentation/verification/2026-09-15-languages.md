...
独立したモデルが Wave D の全 16 言語について、各 292 キーを Apple 用語集と照合して読み直した。
修正は `a742b96`、`dba7658`、`418f66a`、`a75d381`、`3035dd3` に反映済み。
カタログの修正は **39 件**。このほか、チェコ語の Services を **2 項目**修正した。

| 言語 | カタログの修正件数 | Services の修正件数 |
|---|---:|---:|
| `ru` ロシア語 | 9 | 0 |
| `uk` ウクライナ語 | 1 | 0 |
| `pl` ポーランド語 | 2 | 0 |
...
| `sv` スウェーデン語 | 0 | 0 |
| `da` デンマーク語 | 1 | 0 |
| `nb` ノルウェー語（ブークモール） | 1 | 0 |
| `fi` フィンランド語 | 1 | 0 |
| `tr` トルコ語 | 1 | 0 |
| `pt-PT` ポルトガル語（ポルトガル） | 3 | 0 |
| `hi` ヒンディー語 | 10 | 0 |
| `th` タイ語 | 0 | 0 |
| `vi` ベトナム語 | 1 | 0 |
| `id` インドネシア語 | 1 | 0 |
...
- `ru`: 展開の動詞は Archive Utility の「разархивировать」に合わせた。
  「Разблокировать」、男性形の「Стандартный」、選択操作の「Выбрать」も統一した。
- `cs`: 製品名を格変化させず、「pomocí aplikace KaitoFinder」「Nápověda pro KaitoFinder」とした。
  Services の 2 項目も同じ方針で修正した。
- `fi`: ようこそウインドウは「Tervetuloa-ikkuna」とした。
- `tr`: 「KaitoFinder’a Hoş Geldiniz」とした。Finder のトルコ語リソースが
  「Finder’da」「Finder’dan」と後舌母音の接尾辞を使うことを確認した。
- `hi`: Apple の綴り「डाइरेक्टरी」を 7 箇所、「閉じる」を「बंद करें」に修正し、文法を 2 箇所直した。
- `vi`: 「sửa đổi an toàn」の語順に修正した。
- `id`: 終了は「Tutup KaitoFinder」とした。
- `ms`: 削除の動詞を「memadam」とした。

検証はオーケストレータが実施した。各バッチで `WordingAcceptanceTests` **36 件**と
`LayoutOverflowTests` **13 件**が成功した。全件実行は **603 件・失敗 1**で、失敗は
`ArchivePreferencesUITests` に残っていた古いヒンディー語の固定期待値だけだった。
`3035dd3` で期待値を同期し、同クラスの再実行は **7 件・失敗 0**となった。

リリース前には、引き続き母語話者によるレビューを推奨する。
...
## 2026-09-16 に追加した 9 キーの読み合わせ

本日追加した 9 キー(終了の確認 3、開く失敗の見出し、読み込み上限、作成元への保存拒否、
拡張子の要求、ゴミ箱の同一性、混在パスワード)× 26 言語を、Codex に文として読ませた
(意味の一致・既存の語彙との整合・敬体・機械翻訳らしさ・Apple 用語)。指摘は 2 系統で、
どちらも採用した:
- 読み込み上限のエラーが 25 言語で「大きすぎる/複雑すぎるため読めない」に短縮され、原文の
  「読み込みの上限を超えた」が落ちていた → 25 言語を原文どおり両方を述べる形に修正。
- 混在パスワードの案内が da/nb/ru/sv/tr/zh-Hans/zh-Hant で「項目を一件ずつ展開する」とも
  読めた → 「パスワードごとにまとめて、グループ単位で展開する」と明示する形に修正。
- fr の終了確認の「?」の前が U+0020 だった → U+00A0(既存キーと同じ)。
ネイティブの校正の代替ではない。
