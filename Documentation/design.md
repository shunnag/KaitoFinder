...
上表はサイドバー・複数の表示形式を含む拡張構想。現在の文書ウインドウは
`NSWindowController` + `NSOutlineView` のリスト表示で、パスバーは `NSPathControl`、
ステータスは `NSTextField` を下端に配置する。`fullSizeContentView` とスクロールビューの
自動インセットで標準ツールバーの背後まで内容を広げ、列見出しと先頭行の表示領域を確保する。
パスバーは標準の高さを使い、細い区切り線で一覧と分ける。
...
設定専用の標準ツールバー (`toolbarStyle = .preference`) と、システム色の `NSBox` で関連項目をまとめる。
全タブは翻訳に必要な幅を共有し、高さだけを内容に合わせる。切り替え時は上辺を保ち、
画面下端を越える場合は画面内へ移す。「視差効果を減らす」が有効ならサイズ変更をアニメーションしない。
設定の全タブを 26 言語 × ライト / ダークで描画し、実ツールバーの action による切り替えも検査する。

**書庫のタブ**: 一般の「アーカイブを開くとき:」は `ArchiveOpeningBehavior` に
`system`（既定）/ `newTab` / `newWindow` を保存する。関連付け・開く・履歴の共通入口である
`ArchiveWindowController.showWindow` の初回表示直前に読み、`NSWindow.tabbingMode` を
`.automatic` / `.preferred` / `.disallowed` にする。表示後は `.automatic` に戻し、既存ウインドウを
...
既定の配置は通常スペースで「展開」「追加・新規フォルダ・削除」「クイックルック」を分け、
macOS 26 / 27 の標準ツールバーがグループのマテリアルを描く。
展開には `tray.and.arrow.down` を使い、メニューバーとコンテキストメニューの共通操作も
同じ SF Symbol で示す。動的に入力欄が変わるウインドウはキービューループを自動再計算する。
...
**同じ書庫の一覧内のドラッグは移動**とし、⌥ を押したときはコピーにする。
別の書庫へのドラッグは、別ウインドウ・同じウインドウの別タブとも file promise によるコピー。
同一性は物理的なウインドウの枠ではなく `draggingSource === outlineView` で判定する。
選択された祖先がある項目には provider を作らず、フォルダに含まれる子の二重送信を防ぐ。
各 provider で全選択を走査せず祖先だけを調べ、コピー・展開・ステータスの集計も同じ親選択の規則を使う。
`ArchiveIncomingFiles` は drop の同期呼び出し中にすべての promise 受信を開始し、全 callback を
...
「最近使った項目を開く」は `RecentDocumentsMenu.xib` の標準 `recentDocuments` メニューを
ファイルメニューへ組み込み、履歴の更新・再オープン・消去を `NSDocumentController` に任せる。
通常の `NSMenu` に消去 action だけを置いても履歴は表示されない。
[履歴メニューの検証](verification/2026-09-17-recent-documents.md)では、メニューを実際に表示し、
生成された項目から文書を再オープンする回帰テストを記録している。
...
- 保存パネルと全三シートは 26 言語で描画・`UISnapshot.overflowViolations` の対象とする。
  保存パネルでは形式・レベル・パスワードのラベル列を共有する。ZIP / 7z のパスワード欄は
  暗号化がオンのときだけ表示する。On / Off は保存パネルのコンテンツビューの
  `NSView.displayLink` を時計にし、描画コールバックを抜けてから main queue でサイズを反映する。
  0.24 秒の ease-in/ease-out を目標に高さと透明度を更新する。XPC ホストと実パネルに反映済みの
  高さから 8 pt より先へ要求を進めず、反映が遅れた場合は完了まで更新を続ける。
  透明度も反映する高さに合わせ、時刻だけで先に入力欄を消さない。途中の高さは整数ポイントに揃え、
  シートでは増分を偶数ポイントにして中央の丸めによる往復を防ぐ。終点は自然な高さに合わせる。
  `fittingSize` は現在要求している高さを返し、最終サイズの計測中に先にパネルを伸縮させない。
  アクセサリの原点は AppKit に任せる。フォームの上端は、XPC ホストの遅れて届く `bounds` ではなく、
  実際の `NSSavePanel.frame` とアクセサリ以外の高さから計算する。
  `NSWindow.didResizeNotification` でも即座に配置を更新し、内側の位置にはアニメーションを付けない。
  アクセサリ自身は要求中の高さでクリップせず、保存パネルのホストに表示範囲の切り取りを任せる。
  これにより、縮小要求が実ウインドウより先に反映された瞬間にも先頭行を欠けさせない。
  一覧を開いたパネルはアクセサリの縮小分を一覧へ割り当てるため、パネル自体の高さも合わせて更新する。
  この場合は、パネルの表示領域を先に更新してからアクセサリのサイズを反映する。
  同期再描画は強制せず、ウインドウを画面内に収める。画面の高さを超える場合は即時に切り替え、
  一覧の領域調整を標準パネルに任せる。
  一覧の開閉と手動リサイズの開始時は、進行中の切り替えを確定して基準の高さを測り直せる状態にする。
  連続操作では現在の高さから再開する。世代番号で古い描画更新とキュー内の処理を無効にし、
  再表示・再フォーカスを防ぐ。「視差効果を減らす」が有効なら即時に切り替える。
  保存待ちの終了と解放時には display link を停止する。
  入力中の検証メッセージの高さは確保し、エラーの有無ではレイアウトを動かさない。
  長い翻訳ではラベル・popup・チェックボックスの固有幅からフォーム幅を決める。
  アクセサリの外枠は標準保存パネルの幅に合わせて伸縮し、内側のフォームを中央に配置する。
  暗号化の解除や非対応形式への切り替えでは、入力フォーカスを有効なコントロールへ移す。
  オフの間は入力検証メッセージを消し、入力済みの値は再度オンにするまで保持する。
...
- **quarantine**:書庫の `com.apple.quarantine` を展開物へ**伝播**させる。
  ...
  作成・追加時は選択した項目だけでなく**取り込む全ファイルとフォルダ**を調べ、最初に
  見つかった印を生成物に写す。追加では原本の印があればそれを優先する(2026-09-16。
  印のないフォルダに入れた印付きのダウンロードや、子フォルダだけに印のある項目でも
  迂回路にならない)。
- **格納されていないパーミッション**: 通常の展開ではファイル 0666 / フォルダ 0777 に起動時の umask を適用する。
  ファイルは 0600 で作成し、同じ descriptor へ quarantine を適用して本文の CRC 検証が完了してから mode を広げる。
  合成フォルダは子の作成中は 0700 とし、完了時に既定 mode を適用する。読み取り専用の一時コピーは 0400 を保つ。
- **RLO 偽装拡張子**を検出して警告する。
...
231 s → 1 s 未満)。続けて、ファイルごとに root から親ディレクトリを開き直していた固定費を
親 descriptor と親の path 検査のキャッシュで下げ、Release の 50 万件の一括展開の CPU 時間を
327 s → 137〜151 s にした([記録](verification/2026-09-16-extract-throughput.md)。拒否理由は
全て不変。キャッシュは「1 インスタンス = 1 つの同期 worker」に依存し、§12-4 の並列展開では
worker ごとに destination を作る)。

### 11.1 2026-09-16 のブラッシュアップ(標準手順と完了の基準)

**標準手順。** (1) Codex に読み取り専用の敵対的レビューをさせる(対象領域と「操作の順序・失敗の
順序・異常入力・外部変更」の観点。候補ごとに所在・引き金・期待と実際・コード経路・確信度・
テストの骨子。既存テストで担保済みの項目は名前を挙げて除外)。(2) オーケストレータが候補を
コードで確認し、確信度「高」だけを仕様にする。(3) Codex が**修正前に失敗するテスト**を先に
書いてから直す。(4) オーケストレータが実際の `xcodebuild test`・実機の起動スモーク・必要なら
stash による「修正前に失敗」の再確認を行い、記録を `verification/` に残して commit する。
仕様の雛形は scratchpad の `specs/kf-review-*.md`。

**完了の基準(候補 1 件につき)。**
- レビュー側の主張ではなく、オーケストレータがコードを読んで経路を確認していること。
- 修正前に失敗し、修正後に通る回帰テストがあること(新しい API に依存して旧コードでは
  コンパイルできないテストは、その旨を記録に書く)。
- 全件の `xcodebuild test` が通り、AppDelegate / 文書に触れた場合は実機の起動スモークを済ませていること。

**同日の反復(候補数 8 → 7 → 9 → 4)。**
- Wave H — ウインドウ状態機械: [記録](verification/2026-09-16-window-state-review.md)。
...
- 追加の[総合レビュー](verification/2026-09-16-comprehensive-review.md)では、作成元の子孫への
  上書き、追加・子フォルダの隔離属性、`./` 付きパスの編集、台帳の起動・プロセス間競合を修正。
  選択変更時の全件集計を表示フィルタ作成時へ移し、大規模改名・削除の検証とGyoshukuKitの予約を
  索引化した。同じpromiseの重複要求の管理と、結合文字によるパス検証のすり抜けも修正した。
...
5. ~~**クラッシュ・強制終了の残骸。**~~ **解決(2026-09-16、Wave L)。** `.KaitoFinder-add-*`
   ...
   作る前に所有PIDとともに記録し、作った後に dev/ino を記録し、完了で外す。次回起動時に台帳に
   残った項目のうち、生存中の所有プロセスがなく、自分の命名規則・ディレクトリ・dev/ino 一致の
   ものだけを回収する。プロセス内は Mutex、プロセス間は固定ロックファイルの `flock` で、
   読み込みから atomic 保存までを排他する。記録は `verification/2026-09-16-pending-work-registry.md`、
   起動中の新規作業の保護と複数プロセスの検証は `verification/2026-09-16-comprehensive-review.md`。
