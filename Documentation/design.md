...
- **quarantine**:書庫の `com.apple.quarantine` を展開物へ**伝播**させる。
  ...
  作成時は選択した項目だけでなく**取り込む全ファイル**を調べ、最初に見つかった印を
  生成物に写す(2026-09-16。印のないフォルダに入れた印付きのダウンロードを固めても
  迂回路にならない)。
- **RLO 偽装拡張子**を検出して警告する。
...
2026-09-16 の Wave H では、Codex に読み取り専用の敵対的レビュー(操作の順序と非同期の完了順)を
させ、裏付けの取れた 8 件を「修正前に失敗するテスト」を先に書いてから直した
([記録](verification/2026-09-16-window-state-review.md)): 改名確定直後の再入、同名レコードの
選択復元、作成中のようこそウインドウへのドロップ、進捗シート付きのようこそウインドウの close、
開く失敗の見出し、検索中の改名・移動・折り畳み・横スクロールの復元。

同日の Wave I は作成・別名で保存・一括展開・設定・保管庫を同じ方法でレビューし、7 件を直した
([記録](verification/2026-09-16-creation-batch-review.md)): 配下ファイルの隔離属性の伝播、
作成元への上書き拒否、パネル待ち中の原本差し替えの検出、形式に合わない拡張子の拒否、
失効した記憶パスワードの削除、ゴミ箱へ入れる前の同一性確認、取消し回収の inode 確認。

同日の Wave J は取り出し(drag promise・コピー)・QuickLook と一時コピー・undo/redo・外部変更・
パスワード入力を同じ方法でレビューし、6 件を直した
([記録](verification/2026-09-16-extract-undo-review.md)): undo/redo 前の原本の同一性確認、
undo できない削除の確認、QuickLook パネルを閉じても一時コピー機能を閉じない、別インスタンスの
起動時 sweep が生きているプロセスの領域を残す、別名で保存後も受理済みの drag promise を
...
**ブラッシュアップの標準手順(2026-09-16 に確立)。** (1) Codex に読み取り専用の敵対的レビュー
(対象領域と「操作の順序・失敗の順序・異常入力・外部変更」の観点、候補ごとに所在・引き金・
期待と実際・コード経路・確信度・テストの骨子、既存テストで担保済みの項目は名前を挙げて除外)を
させる。(2) オーケストレータが候補をコードで確認し、確信度「高」だけを仕様にする。(3) Codex が
**修正前に失敗するテスト**を先に書いてから直す。(4) オーケストレータが実際の `xcodebuild test`・
実機の起動スモーク・必要なら stash による「修正前に失敗」の再確認を行い、記録を
`verification/` に残して commit する。4 回の反復で候補は 8 → 7 → 9 → 4 と減った。仕様の雛形は
scratchpad の `specs/kf-review-*.md` にある。TSan / ASan の全件実行は
[記録](verification/2026-09-16-sanitizers.md)。
